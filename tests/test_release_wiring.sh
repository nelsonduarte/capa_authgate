...
#
# This package has SEVEN top-level modules and the release used to
# compile two of them. main.capa and service.capa are the two
# front-ends; authgate.capa is the pure verifier under both;
# router.capa is service.capa's request dispatch; example.capa is the
# runnable example the README points at. The two `leaky_*` modules are
# deliberate negatives and appear in the two lists below instead.
ENTRY_POINTS=(main.capa service.capa authgate.capa router.capa example.capa)
...
#
# authgate.capa and router.capa are here even though both already lie
# inside the closure of an entry the release checked before. Being
# inside a closure is not a property anyone can see from a directory
# listing, and it stops being true the moment someone refactors an
# import away; naming them costs one line each and states the coverage
...
# cannot disagree.
CEILING_ENTRIES=(main.capa service.capa authgate.capa router.capa example.capa)
...
#
# leaky_handler.capa is the README's central demonstration: a handler
# whose TYPE cannot declare a capability, capturing `Net` in a closure
# instead. It compiles, and the ceiling is the only thing that refuses
# it. That was documented and never verified until this line existed.
NEGATIVE_CEILING_ENTRIES=(leaky_handler.capa)
...
#
# leaky_verify.capa tries to build a `Net` inside the pure verifier.
# The expected substring is the compiler's own wording, unchanged since
# 1.16.0 and quoted verbatim in the README.
COMPILER_REJECTS=("leaky_verify.capa=capability 'Net' cannot be constructed")
